<template>
  ...
  <div class="chat-container">
    <div class="chat_content" ref="chatRoom" v-html="stats.content"></div>
...
import {getTeamMessage} from "../service/chatApi.js";
...
onMounted(async () => {
  ...
  stats.value.user = await getUser()
  console.log(stats.value.user)
  ...
  if (stats.value.chatType === stats.value.chatEnum.TEAM_CHAT) {
    ...
    let teamMessage = []
    teamMessage = await getTeamMessage(stats.value.team.teamId)
...
const init = () => {
  ...
  if (typeof (WebSocket) == "undefined") {
    ...
    //  let socketUrl = `ws://127.0.0.1:8080/api/websocket/${uid}/${stats.value.team.teamId}`
     let socketUrl = `wss://59.110.12.230:8080/api/websocket/${uid}/${stats.value.team.teamId}`
    if (socket != null) {
      socket.close();
      socket = null;
    }
 ...
 */
 const createContent = (remoteUser, nowUser, text, isAdmin,createTime) => {
  ...
  if (nowUser) {
    ...
    html = `
    <div class="chat_message self">
    <div class="chat_myInfo chat_info">
      <img :alt=${nowUser.username} class="chat_avatar" onclick="showUser(${nowUser.id})" src=${nowUser.avatarUrl}>
    </div>
...
`
    ...
    html = `
     <div class="chat_message other">
      <img :alt=${remoteUser.username} class="chat_avatar" onclick="showUser(${remoteUser.id})" src=${remoteUser.avatarUrl}>
    <div class="chat_info">
      <span class="chat_username">${remoteUser.username.length < 10 ? remoteUser.username : remoteUser.username}&nbsp;&nbsp;&nbsp;${createTime}</span>
